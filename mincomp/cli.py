...
def calculate_optimal_threads() -> int:
    ...
    Leaves 1 core free for system stability if more than 2 cores exist.
    """
    try:
    ...
    if cores <= 2:
        ...
        return cores - 1  # On 3+ cores, leave one core free for OS background tasks
...
def compress_video(
    ...
    cmd = [
        ...
        "-c:v",
        "libx264",  # Use x264 encoder
        "-crf",
        str(crf),  # Constant Rate Factor (lower is better, 28 is high compression)
        "-preset",
        preset,  # Encoder preset speed
        "-c:a",
        "aac",  # AAC audio encoder
        "-b:a",
        audio_bitrate,  # Audio bitrate
        "-threads",
        str(threads),  # Optimized threading
        str(output_path),  # Output file
    ]

    print("--- MinComp: Video Compression ---")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MinComp - Minimal Video Compressor for low-end CPUs."
    )
