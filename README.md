...
2. **Build the minimal FFmpeg binary.** Run the included shell script from the root of the project:
   ...
   ./build_ffmpeg.sh
   ```
   (*This step will download the source code for x264 and FFmpeg, configure them for minimal size/high speed, compile them, and place the final binary in ```mincomp/bin```.*)
